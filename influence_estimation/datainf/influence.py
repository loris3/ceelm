...
class IFEngineGeneration(object):
    ...
    def preprocess_gradients(self, tr_grad_dict, test_grad_dict):
    ...
    def compute_hvp_proposed(self, lambda_const_param=10, batch_size=64):
        start_time = time()
        hvp_proposed_dict = {}
        ...
        for weight_name in tqdm(self.val_grad_avg_dict, desc="Weights"):
            ...
            for i in range(0, n_train, batch_size):
                ...
                batch_grads = torch.stack([self.tr_grad_dict[tr_id][weight_name] for tr_id in batch_ids])
    ...
    def compute_IF(self):
        for method_name in self.hvp_dict:
            ...
            self.IF_dict[method_name] = pd.DataFrame(if_tmp_dict, index=list(self.tr_grad_dict.keys()), columns=list(self.test_grad_dict.keys()), dtype=float).T   
